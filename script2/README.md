...
Running this code block will generate a button that you can use to select a file from your computer. For more examples of ways to upload surveys to Colab see this blogpost. If you are working locally, it's important to make sure that you are working in the right directory. For more information on opening and reading files locally see this web page from Stanford University. Google's documentation for Colab also has a lot of good information about working with files, including how to use Google drive or Google sheets.

Reading the Survey
This next section of code opens the file, processes the file, and outputs the different parts of the string into a sentence for each line in the .txt file. This code uses the split() method to split each line in the file on the hyphen. Before running the whole code block, experiment with the split method.
...
How about "Nairobi County" and "nairobi county"? Take a look at the Python string functions reference and see if you can find a function that could transform these two so they look the same to the computer.

There are lots of functions which could remove the case distinction. str.lower() would convert all the names to all lower case, str.upper() would CONVERT THEM ALL TO UPPER CASE, or str.capitalize() would Capitalize the first letter only. Here’s one possible solution:
